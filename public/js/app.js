$(document).ready(function() {
    ...
    var customer_routes_table = $('#customer_routes_table').DataTable({
        processing: true,
        serverSide: true,
        fixedHeader: false,
        ajax: '/customer-route',
        ...
        columns: [
            { data: 'name', name: 'name', render: function(data, type, row) { return data; } },
            { data: 'slug', name: 'slug' },
            { data: 'description', name: 'description' },
            { data: 'is_active', name: 'is_active', render: function(data, type, row) { return data; } },
            { data: 'child_count', name: 'child_count', render: function(data, type, row) { return data; } },
            { data: 'customer_count', name: 'customer_count', render: function(data, type, row) { return data; } },
            { data: 'action', name: 'action', render: function(data, type, row) { return data; } }
        ],
    });

    $(document).on('submit', 'form#customer_route_add_form', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    $('div.customer_routes_modal').modal('hide');
                    toastr.success(result.msg);
                    customer_routes_table.ajax.reload();
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    $(document).on('click', 'button.edit_customer_route_button', function() {
        $('div.customer_routes_modal').load($(this).data('href'), function() {
            $(this).modal('show');

            $('form#customer_route_edit_form').submit(function(e) {
                e.preventDefault();
                var data = $(this).serialize();
    ...
    $(document).on('click', 'button.delete_customer_route_button', function() {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_customer_group,
            icon: 'warning',
