...
$(document).ready(function() {
    ...
    $(document).on('change', '.payment-amount', function() {
        ...
        // Get the current payment row
        var current_payment_row = $(this).closest('.payment_row');
        var current_payment_amount = __read_number($(this));

        // Calculate total of all other payments
        var total_other_payments = 0;
        $('#payment_rows_div')
            .find('.payment-amount')
            .not($(this))
            .each(function() {
                if (parseFloat($(this).val())) {
                    total_other_payments += __read_number($(this));
                }
            });

        // Check if total payments would exceed total payable
        if (total_other_payments + current_payment_amount > total_payable) {
            // Adjust the current payment amount to not exceed the total payable
            var max_allowed_payment = total_payable - total_other_payments;
            if (max_allowed_payment < 0) max_allowed_payment = 0;

            __write_number($(this), max_allowed_payment);

            // Show a message to the user
            toastr.warning(LANG.paid_amount_is_more_than_payable);
        }
